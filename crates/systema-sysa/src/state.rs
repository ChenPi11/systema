...
pub struct UnitRuntimeInfo {
    ...
    /// Unique invocation ID for the current activation, if any.
    /// Generated by System A for each Start/Restart task and passed to the
    /// worker, which sets the INVOCATION_ID environment variable.
    pub invocation_id: Option<String>,
    /// Reference count from external D-Bus clients (RefUnit/UnrefUnit).
    /// Prevents the unit from being unloaded while >0.
    pub n_ref: u64,
}
...
pub struct WorkerTask {
    ...
    /// Generated for each Start/Restart task; empty for Stop/Reload.
    pub invocation_id: Option<String>,
}
...
impl AllocatorState {
    ...
    pub fn unref_unit(&mut self, source: &str, target: &str) -> bool {
        let existed = self
            .ref_counts
            .get_mut(target)
            .map(|sources| sources.remove(source))
            .unwrap_or(false);
        // Clean up empty entries.
        if let Some(sources) = self.ref_counts.get(target) {
            if sources.is_empty() {
    ...
    /// Return the number of units that reference `target`.
    pub fn ref_count(&self, target: &str) -> usize {
        self.ref_counts
            .get(target)
