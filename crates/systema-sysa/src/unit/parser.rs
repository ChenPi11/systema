...
pub fn parse_unit_from_path(path: &Path) -> Result<UnitFile> {
    ...
    let content = std::fs::read_to_string(path)
        .with_context(|| libsysa::l10n::fmt(
            libsysa::l10n::t_("Reading unit file {path} ..."),
...
fn apply_dropin_dir(dir: &Path, unit: &mut UnitFile) -> Result<()> {
    let mut entries: Vec<_> = std::fs::read_dir(dir)
        .with_context(|| libsysa::l10n::fmt(
            libsysa::l10n::t_("Reading drop-in dir {dir} ..."),
...
fn apply_dropin_content(unit: &mut UnitFile, content: &str) -> Result<()> {
    ...
    config
        ...
        .map_err(|e| anyhow::anyhow!(libsysa::l10n::fmt(
            libsysa::l10n::t_("INI parse error in drop-in: {error}."),
            &[("error", &e.to_string())],
        )))?;
...
fn parse_unit_content(name: &str, content: &str) -> Result<UnitFile> {
    ...
    config
        .read(processed)
        .map_err(|e| anyhow::anyhow!(libsysa::l10n::fmt(
            libsysa::l10n::t_("INI parse error in {name}: {error}."),
    ...
    parse_unit_section(&config, &mut unit.unit, name)
        .with_context(|| libsysa::l10n::fmt(
            libsysa::l10n::t_("Parsing [Unit] section of {name} ..."),
            &[("name", name)],
        ))?;
    ...
    parse_install_section(&config, &mut unit.install)
        .with_context(|| libsysa::l10n::fmt(
            libsysa::l10n::t_("Parsing [Install] section of {name} ..."),
            &[("name", name)],
        ))?;
    ...
    match &unit.kind {
        UnitKind::Service => {
            ...
            parse_service_section(&config, &mut svc, name)
                .with_context(|| libsysa::l10n::fmt(
                    libsysa::l10n::t_("Parsing [Service] section of {name} ..."),
                    &[("name", name)],
                ))?;
        ...
        UnitKind::Mount => {
            ...
            parse_mount_section(&config, &mut mnt, name)
                .with_context(|| libsysa::l10n::fmt(
                    libsysa::l10n::t_("Parsing [Mount] section of {name} ..."),
                    &[("name", name)],
                ))?;
            unit.mount = Some(mnt);
        }
        UnitKind::Timer => {
            ...
            parse_timer_section(&config, &mut tmr, name)
                .with_context(|| libsysa::l10n::fmt(
                    libsysa::l10n::t_("Parsing [Timer] section of {name} ..."),
                    &[("name", name)],
                ))?;
            unit.timer = Some(tmr);
        }
        UnitKind::Socket => {
            ...
            parse_socket_section(&config, &mut sock, name)
                .with_context(|| libsysa::l10n::fmt(
                    libsysa::l10n::t_("Parsing [Socket] section of {name} ..."),
                    &[("name", name)],
                ))?;
            unit.socket = Some(sock);
        }
        UnitKind::Swap => {
            ...
            parse_swap_section(&config, &mut swap, name)
                .with_context(|| libsysa::l10n::fmt(
                    libsysa::l10n::t_("Parsing [Swap] section of {name} ..."),
                    &[("name", name)],
                ))?;
            unit.swap = Some(swap);
        }
        UnitKind::Path => {
            ...
            parse_path_section(&config, &mut path_sec, name)
                .with_context(|| libsysa::l10n::fmt(
                    libsysa::l10n::t_("Parsing [Path] section of {name} ..."),
                    &[("name", name)],
                ))?;
            unit.path = Some(path_sec);
        }
        UnitKind::Slice => {
            ...
            parse_slice_section(&config, &mut slice, name)
                .with_context(|| libsysa::l10n::fmt(
                    libsysa::l10n::t_("Parsing [Slice] section of {name} ..."),
                    &[("name", name)],
                ))?;
            unit.slice = Some(slice);
        }
        UnitKind::Scope => {
            ...
            parse_scope_section(&config, &mut scope, name)
                .with_context(|| libsysa::l10n::fmt(
                    libsysa::l10n::t_("Parsing [Scope] section of {name} ..."),
                    &[("name", name)],
                ))?;
            unit.scope = Some(scope);
        }
        UnitKind::Device => {
            ...
            parse_device_section(&config, &mut device, name)
                .with_context(|| libsysa::l10n::fmt(
                    libsysa::l10n::t_("Parsing [Device] section of {name} ..."),
                    &[("name", name)],
                ))?;
            unit.device = Some(device);
        }
